...
export class Ts01Component {
  variables():void{
    ...
    let a; //Si no dices de qué tipo es luego permite cambiar el tipo
    const PI = 3.1416;
    console.log(PI);
    a = "kenai";
    a = 5;
    a = true;
    let b:string="hola"; // Si dices el tipo no permite cambiar
    let c:boolean | number; // También te permite decir varios tipos
    c=false;
    c=6;
  ...
  tipoString():void{
    ...
    let lorem:string = "       Lorem ipsum, dolor sit amet consectetur adipisicing elit. Est, et dolorum. Adipisci sequi, voluptates accusamus earum magnam non pariatur odit consequuntur dicta modi. Quam soluta aliquid nam ea quod dolore.      ";
        ...
        console.log(saludo,lorem); //métodos del ToString
        console.log(
          saludo.length,
          lorem.length,
          saludo.toUpperCase(),
    ...
    let nombre="Jorge";
    let apellido="López";
    let saludo2 = `Hola mi nombre es ${nombre} ${apellido}.`; //Para no concatenar varias
    console.log(saludo2);
  }
  tipoNumber():void{
    let a = 2;
